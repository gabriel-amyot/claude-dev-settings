...
name: sprint-factory
version: "1.7.0"
description: "Sprint Factory (multi-ticket conductor; formerly 'dark-factory v1', renamed when v2 took the plain dark-factory name). Takes a set of Jira tickets with dependencies (flat list auto-dep-checked, or plan-file with a YAML dependency graph), computes topological tiers, and drives each ticket to dev. The per-ticket lifecycle belongs to dark-factory; Sprint Factory orchestrates the multi-ticket DAG and (target model) conducts each ticket THROUGH dark-factory via handoffs. Idempotent + rerunnable: reconciles from Jira ticket status + the handoff ledger, so a rerun skips in-progress handoffs and unlocks children of closed tickets. For a single ticket, use /dark-factory. Klever-first. Triggers on: '/sprint-factory', 'sprint factory', 'multi-ticket', 'epic to dev', 'dependency graph'."
user_invocable: true
nav:
  bay: build
  when: "Multi-ticket / epic with dependencies: tiered parallel orchestration via a YAML dependency graph. Conducts tickets through dark-factory. Idempotent rerun per sprint."
  when_not: "Single ticket (use /dark-factory). Overnight per-AC autonomous (use sprint-crawl). Quick ship (use /autonomous-ticket-ship)."
  personas: [amelia, quinn, winston]
  org: [klever]
---

> **⚠ Reframe in progress (2026-06-03).** This is **Sprint Factory** — the multi-ticket *conductor*, not
> a per-ticket lifecycle engine. The skill was **renamed** from `dark-factory` to `sprint-factory` (the
> single-ticket factory, formerly "dark-factory v2", took the plain **dark-factory** name). Target model:
> Sprint Factory computes the dependency DAG, fans out one handoff per startable ticket (each runs through
> `/dark-factory`), and unlocks the next tier on report-back. It is **idempotent and rerunnable** — on
> rerun it reconciles from Jira ticket status + the handoff ledger (skip in-progress handoffs, unlock
> children of closed tickets), so you run it once per sprint and can safely re-run if the session closes.
> The 8-phase prose below is **legacy reference** until the orchestration rewrite lands. Design + build
> order: [`docs/sprint-factory-orchestration-model.md`](docs/sprint-factory-orchestration-model.md).
> Still pending (scoped follow-up): the **Mode-1 orchestration rewrite** (the rename itself is now done).

# Sprint Factory (multi-ticket conductor)
