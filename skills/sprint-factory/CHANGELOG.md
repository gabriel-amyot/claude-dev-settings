...
Now that dark-factory owns the single-ticket lifecycle, v1's role is being reframed to the multi-ticket
*conductor*: compute the dependency DAG, fan out one handoff per startable ticket (each runs through
`/dark-factory`), unlock the next tier on report-back. Idempotent + rerunnable: reconciles from Jira
