...
name: ticket-scan
model: sonnet
description: "Ticket health check. Scans acceptance criteria for quality: observable outcomes (not task lists), actor-perspective gaps (can dev implement? can QA assert?), vague language. Works on any ticket (yours or others'). Use when you want to validate AC quality before implementation or check readiness of a ticket. Does NOT modify AC. Does NOT run full quality gate cascade (use story-quality-gate agent for that). Triggers on: 'is this ticket ready', 'check this ticket', 'review the AC', 'how is this ticket looking', 'scan the ticket', 'ticket health'. Input: Jira ticket key. Returns: AC quality report with rewrite suggestions."
tools:
  - Bash
  - Read
  - Write
