...
| `status-update.md` | `body`, `next_steps?` | Jira ticket progress update |
...
5. Do NOT add any attribution: external posts carry no `[automated]` tag, no persona, no model. Keep templates lean (fewest words, no header/section theatre); the engine no longer injects any attribution line.
