...
- **STE anti-slop lint (before preview, advisory).** Run `python3 ~/.claude-shared-config/tools/ste_lint.py <draft-file>` on the draft. Show the score (violations per 100 words) and the offending checks in the preview. Revise obvious hits: hedges, marketing adjectives, phrasal verbs, run-ons, nominalizations. This is advisory for now (no block) while the threshold `N` is calibrated. After calibration the linter becomes a hard gate: it writes a pass-marker and `external-post-gate.sh` will require it. A human may keep a genuine hit with an explicit `Lint-ack: <reason>` line in the draft (quoted error text, vendor name); the agent must not self-ack. See `docs/specs/2026-07-30-ste-anti-slop-design.md`.
