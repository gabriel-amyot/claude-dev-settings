...
each acceptance criterion to specific files, functions, and tests. Backend/Java floor.
...
1. For each affected repo: read its `CLAUDE.md`, grep the code areas the ACs reference (classes,
   endpoints, controllers, validators), confirm the stack (Java/Maven).
...
5. **Multi-module Maven:** if `pom.xml` has `<modules>`, pick the target module(s) from the AC file
   paths and scope all `mvn` commands to those modules (`mvn compile -pl <module>`).
