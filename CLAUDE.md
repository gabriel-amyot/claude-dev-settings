...
- **Fetch-before-read gate (reasoning about deployed code).** Before reasoning about, comparing, or drawing conclusions from local repo code that reflects a deployed environment (e.g. "does dev do X?", "is there a template for Y?", any adversarial code claim): `git fetch` (sequential, never piped), then `git rev-list --count HEAD..origin/<default-branch>`. If the count is > 0, the local checkout is stale — sync (pull) or read `origin/<branch>:path` BEFORE concluding. This applies to READS, not just edits. The same skepticism applies to **derived knowledge docs** (bibliothèque, ADRs, handoffs) that assert what deployed code or data does: treat their claims as hypotheses to verify against live code/BQ, not ground truth, especially when the doc came from unreviewed agent work. Learned from KTP-781 (2026-06-04): a 4-commits-behind checkout produced a false adversarial "COALESCE doesn't exist, no template" conclusion when `origin/dev` had the fix all along; and a bibliothèque doc claimed "the backend county query uses COALESCE" when the deployed adapter did not. Mechanical hook backstop is handed off (sessions handoff: fetch-before-read-guard).
- **Deploy-identity gate (staleness ≠ which branch deploys).** Learned from KTP-688 (2026-06-29): an agent read `main` of `app-agent-hub` and asserted about deployed code that runs `dev` (93 commits apart, opposite architecture), handed wrong line-refs to the code owner, and was embarrassed. `git rev-list HEAD..origin/main = 0` proves "my main is current" — it does NOT prove "main is what deploys." Some repos invert the DAC norm: `app-agent-hub` has `main` as default but `dev` deploys.
  - **Before any claim about DEPLOYED code, confirm which branch deploys.** Run `/deploy-identity` (or `~/.claude/skills/deploy-identity/probe.sh <path>`). To reason about deployed behaviour while on the wrong branch, read it directly: `git -C <repo> show <deploy_branch>:<path>`. Deploy facts live in `~/.claude/deploy-identity/<repo>.yaml`.
  - **Confidence is an OUTPUT of the probe, never self-asserted.** HIGH confidence on a deployed-code claim is permitted only when the probe returns `VERIFIED`. `MISMATCH`/`CANT_VERIFY` caps you at HYPOTHESIS no matter how coherent the story feels. Coherence ≠ verification.
  - **Code-location claims that leave your context carry the stamp.** Cite as `path:line [VERIFIED against dev@<sha>]` or `[UNVERIFIED — read on main, deploy=dev]` so the caveat travels through a human relay. `/post-comment` blocks unstamped code refs.
  - **A challenge is a falsification trigger, not a prompt to gather more confirming evidence.** When a code owner pushes back, dispatch a FRESH-context falsifier (deploy-identity `falsify` mode) tasked with disproving your claim against the deploy branch — self-red-team under the original anchor rubber-stamps.
  - **Mechanical backstops (defense-in-depth):** `deploy-identity-guard.sh` (PreToolUse Read/Grep) blocks reading a non-deploy branch's source during deployed-system reasoning; `challenge-detect.sh` (UserPromptSubmit) injects the falsification directive on owner pushback. Follow the discipline proactively; the hooks only catch what slips.
- **Derived diagnoses are hypotheses until verified (KTP-688 write/read convention).** A confident-sounding sibling RCA/status doc in the SAME ticket is the prior agent's hypothesis, not settled fact — re-verify its load-bearing claim (especially "which branch / what deploys") before building on it. When WRITING: an unverified diagnosis files under an **"Open Questions / Unverified"** heading, never "Verdict." A `Verdict:` line is permitted only with a `verified_against: <commit>` stamp. (Aligns with the existing "a deferred AC is an open question" SOP.)
...
| Next.js static export on Cloudflare Pages, build artifact 404s in prod ("works locally not in prod"), `npx next build` skips npm prebuild hooks, diagnosing CF Pages deploys | `~/.claude/library/context/nextjs-cloudflare-static-export.md` |
# graphify
